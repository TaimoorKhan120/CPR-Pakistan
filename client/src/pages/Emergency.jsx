...
export default function Emergency() {
  ...
  const cancelRequest = async () => {
    ...
    try {
      await fetch(`/api/emergency/request/${request.id}/cancel`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const markComplete = async () => {
    if (!request) return;
    await fetch(`/api/emergency/request/${request.id}/complete`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` },
    });
    if (locationWatchRef.current) {
    ...
    return (
      <div className="p-6 flex flex-col items-center justify-center min-h-[70vh] text-center">
        ...
        <button
          ...
          className="mt-6 bg-pakistan-green text-white px-6 py-3 rounded-xl font-semibold"
        >
          Back to Home
        </button>
      </div>
    ...
    <div className="flex flex-col">
      ...
      <div className="h-64">
        <EmergencyMap
          ...
          providerLocation={state === 'accepted' ? providerLocation : null}
        />
      </div>
      ...
      <div className="p-4 space-y-4">
        ...
        {state === 'accepted' && request && (
          <div className="bg-green-50 border-2 border-green-400 rounded-2xl p-4">
            ...
            <div className="urdu text-green-700 text-xs mb-2">امدادگار آ رہا ہے</div>
            <div className="bg-white rounded-xl p-3">
              <div className="text-sm font-semibold text-gray-800">{request.provider_name}</div>
              <div className="text-xs text-gray-500 mt-0.5">BLS Provider</div>
              {request.provider_phone && (
                <a href={`tel:${request.provider_phone}`} className="mt-2 flex items-center gap-1 text-pakistan-green text-xs font-medium">
                  <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
                    <path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z" />
                  </svg>
                  Call {request.provider_phone}
                </a>
              )}
            </div>
          </div>
        ...
        {state === 'idle' && (
          <div className="flex flex-col items-center py-4">
            ...
            <p className="text-xs text-gray-500 mt-4 text-center max-w-xs">
              Tap to broadcast an emergency alert to all registered BLS providers within 10km
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            onClick={call1122}
            className="bg-pakistan-green text-white rounded-xl p-3 flex items-center gap-2 font-semibold text-sm active:scale-95 transition-transform"
          >
            ...
            <div>
              <div>Call 1122</div>
              <div className="urdu text-xs font-normal opacity-80">فوری کال</div>
            </div>
          </button>
          ...
          <div className="grid grid-cols-2 gap-3 pt-2">
            {state === 'accepted' && (
              <button
                ...
                className="bg-pakistan-green text-white rounded-xl py-3 font-semibold text-sm"
              >
                ✅ Mark Complete
              </button>
            )}
            <button
              ...
              className={`${state === 'accepted' ? '' : 'col-span-2'} bg-gray-100 text-gray-700 rounded-xl py-3 font-semibold text-sm border border-gray-200`}
            >
              Cancel Request
            </button>
          </div>
